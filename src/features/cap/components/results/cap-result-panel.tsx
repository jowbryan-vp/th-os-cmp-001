...
export function CapResultPanel({ scenario, onApply, onSaveOptions, canSaveOptions }: {
  ...
  return <aside className="cap-result-panel" aria-live="polite"><h2>Resultado</h2>{!result ? <p>Configure os itens e calcule o cenário.</p> : <>
    ...
    <div className="cap-save-options"><strong>Comparar no Programa de Necessidades</strong><p>Registre as três áreas agora e escolha uma delas depois.</p><button className="button button--primary" disabled={!canSaveOptions} onClick={() => void onSaveOptions()}>Registrar as 3 áreas</button>{!canSaveOptions && <small>Selecione um item do programa para registrar as opções.</small>}</div>
    <div className="cap-apply"><strong>Ou aplicar uma área agora</strong><button onClick={() => void onApply("minimum")}>Área mínima</button><button onClick={() => void onApply("recommended")}>Área recomendada</button><button onClick={() => void onApply("preliminary_gross")}>Área bruta preliminar</button></div>
  </>}</aside>;
}
