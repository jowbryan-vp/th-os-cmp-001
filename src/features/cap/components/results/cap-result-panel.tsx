...
export function CapResultPanel({ scenario, onApply }: {
  ...
  return <aside className="cap-result-panel" aria-live="polite"><h2>Resultado</h2>{!result ? <p>Configure os itens e calcule o cenário.</p> : <>
    ...
    <div className="cap-apply"><strong>Aplicar ao Programa de Necessidades</strong><button onClick={() => void onApply("minimum")}>Área mínima</button><button onClick={() => void onApply("recommended")}>Área recomendada</button><button onClick={() => void onApply("preliminary_gross")}>Área bruta preliminar</button></div>
  </>}</aside>;
}
