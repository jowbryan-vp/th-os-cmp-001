...
export function HonWorkspace({ projects, initialProjectId, onBack }: { projects: ProjectMasterRecord[]; initialProjectId?: string; onBack: () => void }) {
  ...
  if (loading) return <main className="hon-loading">Carregando Calculadora de Honorários…</main>;
  return <main className="hon-shell">
    <header className="hon-header"><button className="text-action" onClick={onBack}>← Voltar ao CMP-001</button><span className="eyebrow">TH OS · HON-001 · v1.0.0</span><h1>Calculadora de Honorários</h1><p>Formação de preço baseada no custo do serviço. Área é referência de esforço, nunca preço automático por m².</p></header>
    <nav className="hon-tabs" aria-label="Etapas da calculadora">{tabs.map(([id, label]) => <button key={id} aria-current={tab === id ? "page" : undefined} onClick={() => setTab(id)}>{label}</button>)}</nav>
    {notice && <div className="hon-notice" role="status"><span>{notice}</span><button onClick={() => setNotice("")}>Fechar</button></div>}
    <section className="hon-panel">
      {tab === "overview" && <><PanelTitle title="Selecionar projeto e estudo" help="Os dados CMP/CAP são lidos para uma cópia de trabalho; o cadastro mestre não é alterado." />
        ...
        {!study && <button className="button button--primary" disabled={!selectedProject} onClick={() => void newStudy()}>Criar estudo e importar escopo</button>}
      ...
      {tab === "structure" && profile && hourly && <><PanelTitle title="Estrutura do escritório" help="Custos são divididos somente pelas horas faturáveis. Lucro e imposto entram depois." />
        <div className="hon-form-grid"><Field label="Perfil"><select value={profile.id} onChange={(event) => mutate({ structureProfileId: event.target.value })}>{profiles.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><button onClick={() => void duplicateProfile()}>Duplicar perfil</button></div>
      ...
      {tab === "services" && study && <><PanelTitle title="Serviços e estimativa manual de horas" help="A estimativa assistida permanece identificada como sugestão; nenhuma hora foi inventada pelo sistema." />
        <div className="hon-table-wrap"><table><thead><tr><th>Inclui</th><th>Serviço</th><th>Etapa</th><th>Horas estimadas</th><th>Observações</th></tr></thead><tbody>{study.services.map((service) => <tr key={service.id}><td><input aria-label={`Incluir ${service.name}`} type="checkbox" checked={service.included} onChange={(event) => mutate({ services: study.services.map((item) => item.id === service.id ? { ...item, included: event.target.checked } : item) })} /></td><td>{service.name}</td><td>{service.stage}</td><td><input aria-label={`Horas de ${service.name}`} type="number" min="0" step="0.5" value={service.estimatedHours} onChange={(event) => mutate({ services: study.services.map((item) => item.id === service.id ? { ...item, estimatedHours: Number(event.target.value) } : item) })} /></td><td><input aria-label={`Observações de ${service.name}`} value={service.notes} onChange={(event) => mutate({ services: study.services.map((item) => item.id === service.id ? { ...item, notes: event.target.value } : item) })} /></td></tr>)}</tbody></table></div><button onClick={() => void saveStudy()}>Salvar horas</button></>}
      ...
      {tab === "result" && study && <><PanelTitle title="Resultado financeiro" help="Os cinco valores abaixo são conceitos distintos e permanecem rastreáveis." /><div className="hon-form-grid"><Field label="Lucro sobre custo (%)"><input type="number" min="0" value={study.inputs.profitMarkup} onChange={(e) => inputs({ profitMarkup: Number(e.target.value) })} /></Field><Field label="Imposto (%)"><input type="number" min="0" max="99" value={study.inputs.taxPercentage} onChange={(e) => inputs({ taxPercentage: Number(e.target.value) })} /></Field><Field label="Valor comercial manual"><input placeholder="vazio = recomendado" value={study.inputs.commercialPriceCents === null ? "" : inputMoney(study.inputs.commercialPriceCents)} onChange={(e) => inputs({ commercialPriceCents: e.target.value ? parseMoney(e.target.value) : null })} /></Field><Field label="Desconto"><input value={inputMoney(study.inputs.discountCents)} onChange={(e) => inputs({ discountCents: parseMoney(e.target.value) })} /></Field><Field label="Doação"><input value={inputMoney(study.inputs.donationCents)} onChange={(e) => inputs({ donationCents: parseMoney(e.target.value) })} /></Field></div><button className="button button--primary" onClick={() => void calculate()}>Calcular honorários</button>
        {study.results && <><div className="hon-price-ladder"><Metric label="A. Custo do escritório" value={money(study.results.subtotalBeforeProfit)} /><Metric label="B. Mínimo sustentável" value={money(study.results.sustainableMinimum)} /><Metric label="C. Honorário recomendado" value={money(study.results.recommendedFee)} /><Metric label="D. Valor comercial" value={money(study.results.commercialPrice)} /><Metric label="E. Valor final negociado" value={money(study.results.finalPrice)} /></div><div className="hon-summary"><Metric label="Imposto estimado" value={money(study.results.taxes)} /><Metric label="Receita líquida" value={money(study.results.estimatedNetRevenue)} /><Metric label="Lucro estimado" value={money(study.results.estimatedProfit)} /><Metric label="Margem real" value={`${study.results.effectiveMarginPercentage.toFixed(2)}%`} /><Metric label="Acréscimo sobre custo" value={`${study.inputs.profitMarkup}%`} /><Metric label="Hora efetiva" value={money(study.results.effectiveHourlyRate)} /></div>
          ...
          <Warnings items={study.results.warnings} /><div className="hon-actions"><button onClick={exportInternal}>Memória interna JSON</button><button onClick={exportCommercial}>Resumo comercial JSON</button><button onClick={exportCsv}>Serviços CSV</button></div></>}
      </>}

      {tab === "scenarios" && study && <><PanelTitle title="Comparador de cenários" help="Compare até quatro combinações sem substituir o cálculo técnico por média simples." /><button disabled={!study.results || scenarios.length >= 4} onClick={() => void addScenario()}>+ Salvar cenário atual</button>{scenarios.length > 0 && <div className="hon-scenario-grid">{scenarios.map((scenario) => <article key={scenario.id}><h3>{scenario.name}</h3><strong>{money(scenario.result.finalPrice)}</strong><p>{scenario.complexity} · {scenario.urgency}</p><small>{scenario.paymentCondition}</small></article>)}</div>}</>}
      ...
      {tab === "history" && study && <><PanelTitle title="Histórico e snapshots" help="Cálculos aprovados são imutáveis; alterações futuras exigem nova versão." /><div className="hon-summary"><Metric label="Versão do estudo" value={String(study.version)} /><Metric label="Status" value={study.status} /><Metric label="Snapshots" value={String(snapshotCount)} /><Metric label="Atualizado" value={new Date(study.updatedAt).toLocaleString("pt-BR")} /></div><button className="button button--primary" disabled={!study.results || study.status === "approved"} onClick={() => void approve()}>Aprovar snapshot imutável</button></>}

      {tab === "settings" && study && <><PanelTitle title="Configurações e referências" help="Sem tabela técnica formal importada, a referência CAU permanece uma entrada manual identificada pelo arquiteto." /><div className="hon-form-grid"><Field label="Método principal"><select value={study.calculationMethod} onChange={(e) => mutate({ calculationMethod: e.target.value as FeeCalculationStudy["calculationMethod"] })}><option value="service_cost">Custo do serviço</option><option value="cau_reference">Referência CAU/custo da obra</option><option value="commercial_reference">Referência comercial</option></select></Field><Field label="Custo estimado da obra"><input value={inputMoney(study.inputs.constructionCostReferenceCents ?? 0)} onChange={(e) => inputs({ constructionCostReferenceCents: parseMoney(e.target.value) })} /></Field><Field label="Percentual comparativo informado"><input type="number" min="0" value={study.inputs.cauReferencePercentage ?? ""} onChange={(e) => inputs({ cauReferencePercentage: e.target.value ? Number(e.target.value) : null })} /></Field><Field label="Fonte e versão"><input value={study.inputs.cauReferenceSource} onChange={(e) => inputs({ cauReferenceSource: e.target.value })} /></Field><Field label="Área de referência (não define preço)"><input type="number" min="0" value={study.inputs.areaReferenceM2 ?? ""} onChange={(e) => inputs({ areaReferenceM2: e.target.value ? Number(e.target.value) : null })} /></Field><Field label="Notas comerciais"><textarea value={study.inputs.commercialReferenceNotes} onChange={(e) => inputs({ commercialReferenceNotes: e.target.value })} /></Field></div><button onClick={() => void saveStudy()}>Salvar configurações</button></>}

      {!study && tab !== "overview" && <div className="hon-empty"><h2>Crie ou selecione um estudo</h2><p>Comece na Visão geral para vincular um projeto CMP-001.</p><button onClick={() => setTab("overview")}>Ir para Visão geral</button></div>}
    </section>
  </main>;
}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="hon-field"><span>{label}</span>{children}</label>; }
